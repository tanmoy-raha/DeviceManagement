import Vue from 'vue';
Vue.mixin({
  name: 'directivesmixin',
  directives: {
    thousandSeparatorDirMixin: { // directive definition
      componentUpdated(el, binding, vnode, oldVnode) {
        ...
        if ((amount > 0 || amount < 0)) {
          if (amount !== '' || amount !== undefined || amount !== 0 || amount !== '0' || amount !== null) {
            ...
            el.innerText = Number(amount).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
          }
        }
      }
    },
    elementHideDirMixin: { // directive definition
      //hide any element based on blank value
      componentUpdated(el, binding, vnode, oldVnode) {
        let actualvalue = binding.value;
        if (actualvalue !== undefined || actualvalue == '' || actualvalue == null) {
          el.style.display = "none";
        }
      }
    },
    columnsresizable: { // directive definition
      //hide any element based on blank value
      componentUpdated(el, binding, vnode, oldVnode) {
        debugger;
